app.controller("ProductController", function ($scope, $window, $rootScope, DataServices, $routeParams) {
    $rootScope.title = 'RunOut-Biliard | Sản phẩm';
    $window.scrollTo(0, 0);
    ...
    $scope.applyFiltersAndSort = function () {
        if ($routeParams.category_id && $scope.allProducts) {
            $scope.products = $scope.allProducts.filter(function (product) {
                // Kiểm tra product và product.category tồn tại
                return product && product.category && product.category._id == $routeParams.category_id;
            });
            
            var matchedCategory = $scope.categories && $scope.categories.find(function (category) {
                return category && category._id == $routeParams.category_id;
            });
